...
let address = document.querySelector(".selected-cell");
$(document).ready(function () {
  ...
  $(".input-cell").click(function (e) {
    ...
    $(this).addClass("selected");
    addressBarDisplay(this);
    changeHeader(this);
  });

  

  function addressBarDisplay(ele) {
      let [rowId, colId] = getrowcol(ele);
      let row = rowId;
      let col = String.fromCharCode(64 + colId);
      address.value = `${col}${row}`;
  } 
...
function emptySheet() {
  ...
  for (let i of Object.keys(sheetInfo)) {
    for (let j of Object.keys(sheetInfo[i])) {
      ...
      $(`#row-${i}-col-${j}`).css("font-family", "Gill Sans");
